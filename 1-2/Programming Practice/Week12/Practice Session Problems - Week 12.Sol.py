...
class LibrarySearcher():
    ...
    def word_location_dictionary_creator(self):
        ...
        for i in os.listdir(os.curdir):                                 #Looking for all the pages
            ...
            words_in_title = [self.separatewords(words) for words in title_of_paper.split('\n') if words != '']
            words_in_abstract = [self.separatewords(words) for words in abstract_part.split('\n') if words != '']
    ...
    def searcher(self):
        while(True):
            search_terms = raw_input('Please enter your search terms: ')
