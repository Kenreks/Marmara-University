...
class searcher:
    ...
    def getscoredlist(self, results, words, ranking):
        ...
        if (ranking & self.FREQ) != 0:
            weights.append((1.0, self.frequencyscore(results)))

        if (ranking & self.LOC) != 0:
            weights.append((1.0, self.locationscore(results)))

        if (ranking & self.LINK) != 0:
            weights.append((1.0, self.inboundlinkscore(results)))
    ...
    def query(self,q,ranking=NORANK):
        results, words = self.getmatchingpages(q)
        if len(results) == 0:
            print 'No matching pages found!'
            return

        scores = self.getscoredlist(results,words,ranking)
