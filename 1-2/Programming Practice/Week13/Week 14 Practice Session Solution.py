...
class EmotionPredictor(Frame):
    ...
    def initUI(self, parent):
        ...
        self.upload_button = Button(self, text = 'Upload Dataset', font = ('"Helvetica', '12'), command = self.spell_checker)
    ...
    def spell_checker(self):
        ...
        file2 = open("emotion_emojis.csv", "r")
        for i in file2.readlines():
            a, b = i.split(",")[0], i.split(",")[1].strip()
            self.emoji_dict[a] = b
    ...
    def predict(self):
        ...
        for i in best_3_emotion:
            temp_emotion = i[0]
            temp_emoji = emoji.emojize(":%s:" % self.emoji_dict[temp_emotion])
            best_emotion_str = best_emotion_str + temp_emotion + "-->" + temp_emoji + "\n"

        worst_emotion_str = ""
        for i in worst_3_emotion:
            temp_emotion = i[0]
            temp_emoji = emoji.emojize(":%s:" % self.emoji_dict[temp_emotion])
            worst_emotion_str = worst_emotion_str + temp_emotion + "-->" + temp_emoji + "\n"
